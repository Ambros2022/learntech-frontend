...
export default async function sitemap(): Promise<Sitemap> {
  ...
  try {
    const res = await fetch(
      `${apiUrl}/api/website/home/sitemap/universities`,
    ...
    return universities.map((university: any) => ({
      url: `${baseUrl}/university/${university.id}/${university.slug}`,
      lastModified: university.updated_at || new Date().toISOString(),
