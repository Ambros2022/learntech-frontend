...
export async function generateMetadata() {
  const data = await getAbroadCountryPage('study-in-singapore').catch(() => null)
  const url = `${BASE_URL}/study-in-singapore`
  const countryName = data?.country?.name || 'Singapore'
  ...
  const keywords = data?.meta_keyword || `study in singapore, singapore universities, singapore courses, study abroad admission`
