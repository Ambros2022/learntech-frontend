...
export async function generateMetadata({ params }: Props) {
  ...
  const url = `${BASE_URL}/study-in-singapore/${slug[0]}/${slug[1] ?? pagedata.slug}`
  const title = pagedata.meta_title || `${pagedata.name || 'University'} - Study in Singapore | Learntech Edu Solutions`
  ...
  const keywords = pagedata.meta_keyword || `${pagedata.name || ''}, study in singapore, university admission`
...
export default async function Page({ params }: Props) {
  ...
  const [pagedata, testdata, countryData] = await Promise.all([
    ...
    getAbroadCountryPage('study-in-singapore').catch(() => null),
  ])
