...
export async function generateMetadata() {
  const data = await getAbroadCountryPage('study-in-ireland').catch(() => null)
  const url = `${BASE_URL}/study-in-ireland`
  const countryName = data?.country?.name || 'Ireland'
  ...
  const keywords = data?.meta_keyword || `study in ireland, ireland universities, ireland courses, study abroad admission`
