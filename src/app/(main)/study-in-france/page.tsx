...
export async function generateMetadata() {
  const data = await getAbroadCountryPage('study-in-france').catch(() => null)
  const url = `${BASE_URL}/study-in-france`
  const countryName = data?.country?.name || 'France'
  ...
  const keywords = data?.meta_keyword || `study in france, france universities, france courses, study abroad admission`
