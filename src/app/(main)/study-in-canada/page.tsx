...
export async function generateMetadata() {
  const data = await getAbroadCountryPage('study-in-canada').catch(() => null)
  const url = `${BASE_URL}/study-in-canada`
  const countryName = data?.country?.name || 'Canada'
  ...
  const keywords = data?.meta_keyword || `study in canada, canada universities, canada courses, study abroad admission`
