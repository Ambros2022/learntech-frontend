...
export async function generateMetadata() {
  const data = await getAbroadCountryPage('study-in-usa').catch(() => null)
  const url = `${BASE_URL}/study-in-usa`
  const countryName = data?.country?.name || 'USA'
  ...
  const keywords = data?.meta_keyword || `study in usa, usa universities, usa courses, study abroad admission`
