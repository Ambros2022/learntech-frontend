...
export async function generateMetadata() {
  const data = await getAbroadCountryPage('study-in-australia').catch(() => null)
  const url = `${BASE_URL}/study-in-australia`
  const countryName = data?.country?.name || 'Australia'
  ...
  const keywords = data?.meta_keyword || `study in australia, australia universities, australia courses, study abroad admission`
