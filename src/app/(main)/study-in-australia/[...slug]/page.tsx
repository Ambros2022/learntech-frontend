...
export async function generateMetadata({ params }: Props) {
  ...
  const url = `${BASE_URL}/study-in-australia/${slug[0]}/${slug[1] ?? pagedata.slug}`
  const title = pagedata.meta_title || `${pagedata.name || 'University'} - Study in Australia | Learntech Edu Solutions`
  ...
  const keywords = pagedata.meta_keyword || `${pagedata.name || ''}, study in australia, university admission`
...
export default async function Page({ params }: Props) {
  ...
  const [pagedata, testdata, countryData] = await Promise.all([
    ...
    getAbroadCountryPage('study-in-australia').catch(() => null),
  ])
