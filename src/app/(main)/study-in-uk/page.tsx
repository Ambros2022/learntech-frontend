...
export async function generateMetadata() {
  const data = await getAbroadCountryPage('study-in-uk').catch(() => null)
  const url = `${BASE_URL}/study-in-uk`
  const countryName = data?.country?.name || 'UK'
  ...
  const keywords = data?.meta_keyword || `study in uk, uk universities, uk courses, study abroad admission`
