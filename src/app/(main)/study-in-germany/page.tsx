...
export async function generateMetadata() {
  const data = await getAbroadCountryPage('study-in-germany').catch(() => null)
  const url = `${BASE_URL}/study-in-germany`
  const countryName = data?.country?.name || 'Germany'
  ...
  const keywords = data?.meta_keyword || `study in germany, germany universities, germany courses, study abroad admission`
