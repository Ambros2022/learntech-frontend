...
const AddEditForm: FC<Authordata> = ({ olddata, isAddMode, ...rest }) => {
  ...
  const onSubmit = async (data: any) => {
    ...
    if (!isAddMode && olddata.id) {
      ...
      if (data.sub_streams_id.id && data.sub_streams_id.id) {
  ...
  const faqdefaultValues = {
    faqs: isAddMode
      ...
      : olddata.generalcoursefaqs && olddata.generalcoursefaqs.length > 0
      ? olddata.generalcoursefaqs
      : [
          {
            questions: '',
            answers: ''
        ...
        <CardContent>
                    ...
                    render={({ field }) => (
                      <CustomAutocomplete
                        ...
                        renderInput={(params: any) => (
                          <CustomTextField
                            {...params}
                            error={Boolean(errors.sub_streams_id)}
                            {...(errors.sub_streams_id && { helperText: 'This field is required' })}
                            label='Select Sub Stream'
                          />
                        )}
                      />
                    )}
          ...
          <TabPanel sx={{ p: 0 }} value='account-details'>
            {isAddMode ? (
              <>
                {' '}
                <h6>Please add General Course First.</h6>{' '}
              </>
            ) : (
              <>
                      ...
                      <Grid item xs={12}>
                        <Grid container spacing={2} alignItems='center'>
                          <Grid item xs={12} sm={5}>
                            <CustomTextField fullWidth label='Questions' placeholder='' />
                          </Grid>
                          <Grid item xs={12} sm={5}>
                            <CustomTextField fullWidth label='Answers' placeholder='' />
                          </Grid>
                        </Grid>
                      </Grid>
                    )}
