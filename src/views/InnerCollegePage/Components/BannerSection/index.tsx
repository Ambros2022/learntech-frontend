...
import { LazyGlobalEnquiryForm, LazyGlobalEnquiryFormBrochure } from 'src/app/components/ClientWrappers'

const IMG_BASE = (process.env.NEXT_PUBLIC_IMG_URL || '').replace(/\/+$/, '')

function StarRating({ rating }: { rating: number }) {
  return (
    <div className="d-flex gap-2 justify-content-md-end justify-content-start align-items-center">
      {[1, 2, 3, 4, 5].map(n => (
        <i key={n} className={`bi ${rating >= n ? 'bi-star-fill text-warning' : rating >= n - 0.5 ? 'bi-star-half text-warning' : 'bi-star-fill text-white'}`} />
      ))}
        ...
        <div className="card w-100 mb-3 collegeDetailCard">
          <div className="row g-0">
            <div className="col-lg-2 col-xl-1 text-center col-md-2 d-flex justify-content-between">
              ...
              {data?.avg_rating > 0 && (
                <div className="d-flex justify-content-center align-items-center d-md-none col-lg-3 col-xl-3 col-md-10 pt-lg-3 ms-md-auto mb-md-3 mb-3 ps-md-3 ps-0">
                ...
                <h6 className="mb-3 d-flex">
                  ...
                  <div>
                    {data.collegerecognitions?.map((el: any, i: number) =>
                      (i === 0 ? ' ' : ', ') + el.clgrecognitions.recognition_approval_name
                    )}
                  </div>
                </h6>
            ...
            {data?.avg_rating > 0 && (
              <div className="d-none d-md-block col-lg-3 col-xl-3 col-md-10 pt-lg-3 ms-md-auto mb-md-3 mb-3 ps-md-3 ps-0">
                <StarRating rating={data.avg_rating} />
              </div>
            )}
          </div>
          ...
          <div className="d-flex justify-content-end flex-md-row flex-column gap-1 gap-md-3 ps-md-0 ps-0 align-items-center">
            <LazyGlobalEnquiryFormBrochure pagename="Brochure" className="align-content-center btn downloadBtn" title="Download Brochure" collegeName={data.name} />
            <LazyGlobalEnquiryForm className="align-content-center btn freeBtn" buttonText="Get Fee Structure" collegeName={data.name} />
          </div>
        </div>
      </div>
    </section>
