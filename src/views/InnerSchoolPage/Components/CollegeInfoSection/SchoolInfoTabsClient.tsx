...
export interface SchoolTabData {
  id: string
  label: string
  html?: string
  imageUrl?: string
...
export default function SchoolInfoTabsClient({ tabs, schoolName, schoolId }: Props) {
  ...
  return (
    <div className="container position-relative innerClgCarousel">
      ...
      <div className="tab-content pt-5">
            ...
            <div className="col-12 col-lg-4 ps-lg-3 mt-4 mt-lg-0">
              {active.imageUrl && (
                <div className="position-relative innerClgImg2 mb-md-5 mb-3">
                  <Image src={`${IMG_BASE}/${active.imageUrl}`} alt={schoolName} width={700} height={700} loading="lazy" className="rounded img-fluid w-100" style={{ objectFit: 'cover' }} />
                  <div className="position-absolute" style={{ top: '50%', left: '50%', transform: 'translate(-50%, -50%)' }}>
                    <LazyGlobalEnquiryForm className="viewMoreCollegeBtn btn-success2" placeholder="Class" collegeName={schoolName} />
                  </div>
                </div>
              )}
              {active.videoUrl && (
                <div className="clgVideo rounded">
                  <iframe width="100%" height="100%" className="rounded" src={active.videoUrl} title="School video" frameBorder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerPolicy="strict-origin-when-cross-origin" allowFullScreen />
                </div>
              )}
            </div>
          </div>
        ...
        {activeTab === 'gallery' && active?.gallery && (
          <div className="row">
            {active.gallery.map((item, i) => (
              <div key={i} className="col-12 col-md-4 p-2 p-md-4">
        ...
        {activeTab === 'review' && <LazyReviewSec entityId={schoolId} entityName={schoolName} />}
