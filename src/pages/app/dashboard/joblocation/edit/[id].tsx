...
import AddEditForm from 'src/views/app/dashboard/joblocation/AddEditForm'
...
const Edituserlayout = () => {
  ...
  const getolddata = useCallback(async () => {
    try {
      ...
      const response = await axios1.get('api/admin/joblocations/get/' + id);
  ...
  return (
    <>
        ...
        <Card>
          ...
          >
            <h5>Edit Job Location</h5>
            <Link href={`../`} >
              <Button variant='contained'>View All joblocation</Button>
            </Link>
          </Box>
          <CardContent>
