...
const RowOptions = ({ id, onReloadPage }: { id: number | string, onReloadPage: () => void }) => {
  ...
  const DeleteRow = async () => {
    try {
      await axios1.post('api/admin/joblocations/delete/' + id)
        .then(response => {
...
const SecondPage = () => {
  ...
  const [searchfrom, setSearchfrom] = useState<any>('alljoblocations.name')
  const [columnname, setColumnname] = useState<string>('alljoblocations.name')
  ...
  let columns: GridColDef[] = [
...

    {
      flex: 0.2,
      minWidth: 200,
      field: 'alljoblocations.name',
      headerName: 'Job Location',
      renderCell: (params: GridRenderCellParams) => {
        const { row } = params;
        return (
          <Typography noWrap variant='body2' sx={{ color: 'text.primary', fontWeight: 600 }}>
            {row.alljoblocations.name}
          </Typography>
        );
      }
    },
  ...
  const fetchTableData = useCallback(
    async (orderby: SortType, searchtext: string, searchfrom: any, size: number, page: number, columnname: string) => {
      ...
      await axios1
        .get('api/admin/joblocations/get', {
