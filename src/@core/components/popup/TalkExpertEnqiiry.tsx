...
export default function TalkExpertEnquiry({
  type,
  defaultCollegeName,
  defaultCourseName,
  coursePlaceholder = 'Enter Interested Course',
}: Props) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="container w-75 m-auto">
      <div className="row mb-3">
        ...
        <div className="col-lg-6 col-md-12 mb-3 px-xl-4 px-lg-3 px-md-5">
          <input type="text" placeholder={coursePlaceholder} className="form-control" {...register('course')} />
          {errors.course && <div className="error text-danger">{errors.course.message}</div>}
        </div>
        <div className="col-lg-6 col-md-12 mb-3 px-xl-4 px-lg-3 px-md-5">
          <input type="text" placeholder={type ?? 'College Name'} className="form-control" {...register('college_name')} />
        </div>
      </div>
      <div className="text-center px-xl-4 px-lg-3 px-md-3 px-1">
        <button type="submit" disabled={isSubmitting} className="btn-success btn">Submit</button>
      </div>
    </form>
  )
}
